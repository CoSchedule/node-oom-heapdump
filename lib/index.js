      ...
      if (stats.gctype === 2 && !this._busy) {
        ...
        if (memoryUsagePercentage > this._opts.threshold) {
          if (this._count < this._opts.limit) {
            ...
            let oomAfterHeapSnapshotHandlerComplete = false;
            this.createHeapSnapshot(this._opts.path, "OoM")
              .then(this._opts.oomAfterHeapSnapshotHandler)
              .then(() => oomAfterHeapSnapshotHandlerComplete = true)
              .catch((err) => console.error("OoM heapdump: createHeapSnapshot failed:", err));
            ...
            while (!oomAfterHeapSnapshotHandlerComplete && (diff < this._opts.oomBlockMs));
